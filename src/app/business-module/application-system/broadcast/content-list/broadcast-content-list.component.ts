...
import {FileTypeEnum, ProgramStatusEnum, ProgramTypeEnum} from '../../share/enum/program.enum';
 ...
 */
@Component({
  selector: 'app-broadcast-content-list',
  templateUrl: './broadcast-content-list.component.html',
  styleUrls: ['./broadcast-content-list.component.scss']
})
export class BroadcastContentListComponent implements OnInit, OnDestroy {
  /**
   * 节目文件自定义表单
   */
  ...
  private marks = {
    ...
    100: {
      ...
      label: '<strong>' + this.endTime + '</strong>'
    }
  };
    ...
    this.tableConfig = {
      ...
      topButtons: [
        ...
        },
        {
          ...
          needConfirm: true,
          canDisabled: true,
          permissionCode: '09-6-4-3',
          confirmContent: `${this.language.frequentlyUsed.confirmDelete}?`,
          handle: (data: ContentListModel[]) => {
            // // 获取当前部门  才能进行操作，admin是特殊用户 才会做此操作 只能预览
            // const currentUserId = SessionUtil.getUserId();
            // // 是否是当前部门 或者审核人是自己
            // if (data.find(item => !this.isExistsOwnDept(item.deptCode) && item.reviewedUser !== currentUserId)) {
            //   this.$message.warning(this.language.contentList.otherDepartments);
            //   return;
            // }
      ...
      moreButtons: [
        ...
        // 启用
        {
          ...
          needConfirm: true,
          canDisabled: true,
          permissionCode: '09-6-4-4',
          confirmContent: `${this.language.frequentlyUsed.confirmEnable}?`,
          handle: (data: ContentListModel[]) => {
            // 不是同一部门的不可以进行操作
            // if (data.find(item => item.deptId !== SessionUtil.getUserInfo().deptId)) {
            //   this.$message.warning(this.language.contentList.otherDepartments);
            //   return;
            // }
            // 只有已审核或已禁用的节目才可以启用
  ...
  private refreshData(): void {
    ...
    if (!programTypeIsExist) {
      this.queryCondition.filterConditions.push({
        filterValue: ProgramTypeEnum.broadcast,
  ...
  /**
   *
   * @param deptCode 当前节目的DeptCode
   */
  private isExistsOwnDept(deptCode: string): boolean {
    const currentDeptCode = SessionUtil.getUserInfo().department.deptCode;
    return new RegExp(`^${currentDeptCode}`, 'g').test(deptCode);
  }
  ...
  public openContent(jumpType: string, programId?: string): void {
    this.$router.navigate([`business/application/broadcast/broadcast-content-list/${jumpType}`], {
  ...
  public preview(data: ContentListModel): void {
    ...
    const index = data.duration.indexOf('秒');
    const str  = data.duration.substring(0, index) ;
    this.duration = Number(str);
    if ( this.duration > 59) {
      const min = Math.floor(this.duration / 60);
      const sec = this.duration % 60;
      this.endTime = min + ':' + sec;
    } else {
      this.endTime = 0 + ':' + this.duration;
    }
    this.marks = {
      0: {
        style: {
  ...
  public onToExamineOk(): void {
    ...
    const examineParameter = {
      ...
      programType: ProgramTypeEnum.broadcast,
    };
  ...
  private handelExportProgramList(event: ListExportModel<ContentListModel[]>): void {
    ...
    const programTypeFilter = new FilterCondition('programType', OperatorEnum.eq, ProgramTypeEnum.broadcast);
