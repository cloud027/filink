...
@Component({
  selector: 'app-dismantle-percent',
  templateUrl: '../../template-work-order.html',
  styleUrls: ['../../template-work-order.scss']
})
export class DismantlePercentComponent extends WorkCommon {
  ...
  public initTableInspectionConfig(): void {
    const columnConfigs = [
      {title: this.wLanguage.areaDemolitionWorOrderNumber, key: 'count', width: 350, searchable: true, searchConfig: {type: 'input'}},
      {title: this.wLanguage.areaDemolitionWorOrderProportion, key: 'percent', width: 300, searchable: true, searchConfig: {type: 'input'}}
    ];
    this.initPublicTableConfig(300, columnConfigs, 'dismantleAreaPercentExport', undefined, ExportWorkOrderPercentEnum);
  }
  ...
  public statistical(): void {
    ...
    this.refreshPercentData({'queryType': '1', 'procType': '5'});
  }
}
